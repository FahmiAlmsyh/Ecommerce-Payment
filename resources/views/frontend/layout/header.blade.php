    ...
    <div class="container mx-auto py-5">
        <div class="flex flex-stretch items-center">
            <div class="w-56 items-center flex">
                <a href="{{ route('index') }}">
                <img src="{{ asset('asset') }}/images/content/logo.png"
                        ...
                        @if (Auth::user()->roles == 'ADMIN')
                            <li class="mx-3 py-6 md:py-0">
                                <a href="{{ route('dashboard') }}"
                                    class="text-black {{ \Route::current()->getName() == 'index' ? 'md:text-white' : '' }} hover:underline">
                                    Dashboard
                                </a>
                            </li>
                        @else
                            <li class="mx-3 py-6 md:py-0">
                                <form action="{{ route('logout') }}" method="post">
                                    @csrf
                                    <button type="submit" class="btn btn-primary {{ \Route::current()->getName() == 'index' ? 'md:text-white' : '' }}">Logout</button>
                                </form>
                            </li>
                        @endif
                    @endauth
